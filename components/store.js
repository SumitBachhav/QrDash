...
export const useCardData = create((set) => ({
  ...
  addData: (title, text) => set((state) => ({
    cardData: [...state.cardData, { id: state.lastCardId + 1, title: title, value: text }],
...
export const useSelectedCard = create((set) => ({
  ...
  // checkSelectedCard: (id) => set((state) => ({ selectedCard: state.selectedCard?.includes(id) }))
  resetSelectedCard: () => set((state) => ({ selectedCard: [] })),
}))
